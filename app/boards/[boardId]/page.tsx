...
export default async function BoardPage({ params }: BoardPageProps) {
	...
	const completedTasks =
		board.columns.find(
			(col) => col.position === 2 || col.title.toLowerCase().includes("done"),
		)?.tasks.length || 0;
						...
						<div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 lg:gap-6 fade-in-left w-full lg:w-auto">
							<Link href="/" className="self-start sm:self-auto">
								<Button
									variant="retro"
									size="sm"
									className="bg-amber-600 hover:bg-amber-700 bounce-in touch-target"
								>
							...
							<div className="flex items-center gap-3 sm:gap-4">
								<div
									className="bg-amber-700 p-2 sm:p-3 rounded-full border-2 sm:border-3 border-amber-500 shadow-lg bounce-in"
								...
								<div>
									<h1
										className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-wider text-amber-100 drop-shadow-lg fade-in-up"
									...
									{board.description && (
										<p
											className="text-amber-200 text-sm sm:text-base lg:text-lg font-medium tracking-wide mt-1 fade-in-up"
						...
						<div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 lg:gap-6 fade-in-right w-full lg:w-auto">
							...
							<div className="flex items-center gap-2 sm:gap-3 lg:gap-4">
								<div className="text-center bg-amber-600/30 dark:bg-amber-600/40 px-2 sm:px-3 py-1 sm:py-2 rounded-lg border border-amber-500 dark:border-amber-400 pulse-glow stagger-item">
									<div className="text-lg sm:text-xl lg:text-2xl font-bold text-white dark:text-amber-100 drop-shadow">
										{totalTasks}
									</div>
									<div className="text-xs text-white dark:text-amber-100 font-medium">
										総クエスト
									</div>
								</div>
								<div className="text-center bg-amber-600/30 dark:bg-amber-600/40 px-2 sm:px-3 py-1 sm:py-2 rounded-lg border border-amber-500 dark:border-amber-400 pulse-glow stagger-item">
					...
					<div className="retro-card rounded-lg sm:rounded-xl shadow-lg border-2 border-amber-300 p-4 sm:p-6">
						<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
							<div className="flex items-center gap-3">
								<Sword
									className="h-5 w-5 sm:h-6 sm:w-6 text-amber-600 bounce-in flex-shrink-0"
